...
For details about job names, see `v3_naming`_.
...
`v3_naming`_ for how to name jobs.  Adhere to these so that we may
...
run tox and fetch logs and results. It has parameters you can set to control
its behavior, see the `description in zuul-jobs
...
Refer to `central-config-exceptions` for up to date info on which jobs
...
.. _v3_naming: https://docs.opendev.org/opendev/infra-manual/latest/drivers.html#consistent-naming-for-zuul-jobs
